...
export const DropDown = ({
    ...
    const onClick: React.MouseEventHandler<HTMLDivElement> = () => {
        ...
        let element = refDropDownHeader?.current as HTMLDivElement
    ...
    const onClickClose = useCallback<((this: GlobalEventHandlers, ev: MouseEvent) => any)>((ev)=>{
        if(active){
            let element = ev.target as HTMLDivElement
            if(refDropDownHeader.current?.contains(element) || refDropDownBody.current?.contains(element)){
                return;
            }
            onChange()
        }
    ...
    return (
        <>
            ...
            >
                <div
                    className={`fenext-dropdown-header-content `}
                >
                    {header}
                </div>
                <div
                    className={`fenext-dropdown-header-icon `}
                >
                    {loader ? (
                        <>
                            <Loader />
